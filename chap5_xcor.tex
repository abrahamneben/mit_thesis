...
\chapter{Foreground and sensitivity analysis for broad band 21\,cm--Ly$\alpha$ and 21\,cm--H$\alpha$ correlation experiments probing the Epoch of Reionization}
\label{chap:xcor}

Brian Stalder performed the infrared observations and the rough calibration and flat-fielding. John Tonry gave advice on infrared observations. I did all other analysis and wrote the paper.

THE TEXT IN THIS SECTION WAS COPIED FROM XCOR-PAPER-REV4 ON APRIL 2, 2017. IT NEEDS TO BE UPDATED TO MATCH THE CURRENT VERSION OF THAT PAPER.

ALSO I NEED TO RECOPY THE IMAGES FROM THE PAPER TO THIS THESIS FOLDER.

A detection of the predicted anticorrelation between $21$\,cm and either Ly$\alpha$ or H$\alpha$ from the Epoch of Reionization (EOR) would be a powerful probe of the astrophysics of the EOR. While 3D intensity maps isolate foregrounds in the lowest $k_\parallel$ modes, infrared surveys cannot yet match the field of view and redshift resolution of radio intensity mapping experiments. In contrast, 2D (i.e., broad band) infrared intensity maps can be measured with current experiments and are not sensitivity limited. We find that 2D experiments can measure of order 100\% of the 3D fluctuation power at $k<0.2$\,Mpc$^{-1}$ while preserving the correlation properties. However, we show that foregrounds pose two challenges: (1) simple geometric effects result in percent-level correlations between fluxes in these two bands, even if their luminosities are uncorrelated; and (2) the largely uncorrelated radio and infrared foreground residuals contribute a sample variance noise to the cross spectrum. The first challenge demands better foreground masking and subtraction, while the second requires measurements over large fields of view with many independent samples to average down uncorrelated radio and infrared power. Using radio and infrared observations from the Murchison Widefield Array (MWA) and the Asteroid Terrestrial-impact Last Alert System (ATLAS) we set an upper limit on the 21\,cm--Ly$\alpha$ power spectrum at $z\sim7$ of $\Delta^2<181$\,$(\text{kJy/sr}\cdot \text{mK})$ (95\%) at $\ell\sim800$. We predict the levels of foreground flux correlation and the sample variance noise in several possible future experiments, and show that higher resolution radio and infrared surveys such as LOFAR, SKA-LOW, Hubble, Dark Energy Survey can start to probe optimistic models of the 21\,cm--Ly$\alpha$ EOR cross spectrum.
...
\caption[Measured cross spectrum between 185\,MHz and 850\,nm images after our best foreground subtraction and masking.]{Measured cross spectrum between 185\,MHz and 850\,nm images after our best foreground subtraction and masking (red markers). All are consistent with zero, and roughly half are positive (circles) and negative (triangles), as expected if there is no measured correlation. Error bars show $2\sigma$ uncertainties, and we take the tops of these error bars as upper limits on the cross spectrum of 21\,cm--Ly$\alpha$ emission from the EOR. We achieve an upper limit of $\Delta_{21,\text{Ly}\alpha}\lesssim17\,(\text{kJy/sr}\cdot \text{mK})^{1/2}$ (95\%) at all except the largest $\ell$ bin. We also plot the radio--infrared foreground cross spectrum that a 1\% flux correlation would produce (green line), and find that it is below the sensitivity of our MWA--ATLAS experiment.}
\label{fig:resxspec}
\end{figure}
